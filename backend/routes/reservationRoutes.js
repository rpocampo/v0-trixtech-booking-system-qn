...
const ReservationAnalytics = require('../models/ReservationAnalytics');
...
// Check availability for equipment at specific date/time
router.get('/check-availability/:equipmentId', authMiddleware, async (req, res, next) => {
  try {
    ...
    const lockKey = lockService.generateBookingLockKey(equipmentId, date);
    ...
    const result = await lockService.withLock(lockKey, lockOwner, async () => {
      ...
      let availableQuantity = equipment.quantity || 1;
      let reason = '';
      ...
      const serviceRequiresDelivery = requiresDeliveryTruck(equipment);
      ...
      if (equipment.category === 'equipment') {
        ...
        availableQuantity = Math.max(0, equipment.quantity - totalBooked);
...
router.post('/', authMiddleware, async (req, res, next) => {
  try {
    const { serviceId, quantity, bookingDate, notes, deliveryTime, isPackage, packageId, eventLocation } = req.body;
    ...
    if (isPackage && packageId) {
      ...
      for (const inclusion of pkg.inclusions) {
        const includedService = await Equipment.findById(inclusion.serviceId);
        if (!includedService) {
      ...
      const packageBooking = new Reservation(packageBookingData);
    ...
    const service = await Equipment.findById(serviceId);
    if (!service) {
    ...
    // Validate location distances if event location is provided
    ...
    if (eventLocation?.coordinates) {
      ...
      distanceValidation = await locationService.validateBookingDistances(bookingData);

      // Check if location validation is required and failed
      const SystemConfig = require('../models/SystemConfig');
      const config = await SystemConfig.getConfig();

      if (config.location.validation.requireEventLocation && !distanceValidation.valid && config.location.validation.strictDistanceValidation) {
        return res.status(400).json({
          success: false,
          message: 'Location validation failed: ' + distanceValidation.message,
          locationValidation: distanceValidation,
          canProceed: false
        });
      }
    }
    ...
    if (isAvailable) {
      ...
      const bookingResult = await lockService.withLock(bookingLockKey, bookingLockOwner, async () => {
        ...
        const bookingData = {
          ...
          distanceTracking: distanceValidation ? {
            serviceToEvent: distanceValidation.distances?.serviceToEvent || 0,
            userToEvent: distanceValidation.distances?.userToEvent || 0,
            serviceToUser: distanceValidation.distances?.serviceToUser || 0,
            maxAllowedDistance: distanceValidation.maxAllowedDistance,
            distanceValidated: distanceValidation.valid,
            distanceValidationMessage: distanceValidation.message
          } : null,
        };
...
router.get('/delivery-schedules', authMiddleware, async (req, res, next) => {
  try {
    ...
    const allBookings = await Reservation.find(query)
      .populate('serviceId', 'name category serviceType requiresDelivery')
    ...
    const customerSchedules = deliveryBookings.map(booking => {
...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    const bookings = await Reservation.find({ customerId: req.user.id })
      .populate('serviceId')
      .sort({ createdAt: -1 });
...
router.put('/:id/cancel', authMiddleware, async (req, res, next) => {
  try {
    const booking = await Reservation.findById(req.params.id).populate('serviceId');
  ...
  try {
    ...
    if (packages.length < 3) {
      ...
      for (const cat of defaultCategories) {
        ...
        const mainService = await Equipment.findOne({
          category: cat,
          isAvailable: true,
...
const createPackageBooking = async (userId, intent, paymentId) => {
  try {
    ...
    for (const inclusion of pkg.inclusions) {
      ...
      const serviceBooking = new Reservation(serviceBookingData);
      await serviceBooking.save();
      packageBookings.push(serviceBooking);
...
// Validate booking location distances
router.post('/:id/validate-location', authMiddleware, async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    if (!eventLocation?.coordinates) {
      return res.status(400).json({ success: false, message: 'Event location coordinates required' });
    }

    const locationService = require('../utils/locationService');

    const bookingData = {
      serviceLocation: booking.serviceId?.serviceLocation,
      eventLocation,
      userLocation: booking.customerId?.location
    };
...
// Update booking location information
router.put('/:id/location', authMiddleware, async (req, res, next) => {
  try {
    ...
    // Update booking with location and distance information
    booking.eventLocation = eventLocation;
    booking.distanceTracking = {
      serviceToEvent: validation.distances?.serviceToEvent || 0,
      userToEvent: validation.distances?.userToEvent || 0,
      serviceToUser: validation.distances?.serviceToUser || 0,
      maxAllowedDistance: validation.maxAllowedDistance,
      distanceValidated: validation.valid,
      distanceValidationMessage: validation.message
    };
    ...
    res.json({
      ...
      booking: {
        ...
        distanceTracking: booking.distanceTracking
      },
      validation
    });
  } catch (error) {
...
// Get booking location information
router.get('/:id/location', authMiddleware, async (req, res, next) => {
  try {
    const { id } = req.params;

    const booking = await Booking.findById(id)
      .populate('serviceId', 'name serviceLocation')
      .populate('customerId', 'name location')
      .select('eventLocation distanceTracking serviceId customerId');
...
// Validate location before booking creation
router.post('/validate-location-prebooking', authMiddleware, async (req, res, next) => {
  try {
    const { serviceId, eventLocation } = req.body;

    if (!serviceId || !eventLocation?.coordinates) {
      return res.status(400).json({
        success: false,
        message: 'Service ID and event location coordinates required'
      });
    }
    ...
    console.error('Pre-booking location validation error:', error);
    next(error);
  }
});
